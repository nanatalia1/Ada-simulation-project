...
procedure Simulation is
   ...
   Product_Name: constant array (Product_Type) of String(1 .. 10)
     := ("Cake      ", "Vanilla   ", "Strawberry", "Pumpkin   ", "Chocolate ");
   Assembly_Name: constant array (Assembly_Type) of String(1 .. 17)
     := ("Vanilla Cake     ", "Strawberry Cookie", "Pumpkin Cake     ");
   ...
   task body Producer is
      subtype Production_Time_Range is Integer range 4 .. 7;
      ...
      loop
         ...
         loop
            select
              ...
              Put_Line(Product_Name(Product_Type_Number)
                  & " number " &  Integer'Image(Product_Number) & "   has to wait to be accepted in the buffer "); --czekanie, aby uniknac marnowania produktu
               delay Duration(2.0);
           end select;
         end loop;
      end loop;
   ...
   task body Consumer is
      subtype Consumption_Time_Range is Integer range 5 .. 8;
      ...
      B.Deliver(Assembly_Type, Assembly_Number);
         loop
            ...
            if Assembly_Number = 0 then
               delay Duration(3.0);
               Put_Line(Consumer_Name(Consumer_Nb) & "  tries another time for the  " & Assembly_Name(Assembly_Type) ); --ponowna proba
               B.Deliver(Assembly_Type, Assembly_Number);
           -- B.Deliver(Assembly_Type, Assembly_Number);
            else
              Put_Line(Consumer_Name(Consumer_Nb) & " got their  " & Assembly_Name(Assembly_Type) ); --skonczone zamowienie
               exit;
            end if;
         end loop;
      else
         Put_Line(Consumer_Name(Consumer_Nb) & " is waiting in the line"); --zajety watek
         delay Duration(2.0);
           end select;
         end loop;
   end Consumer;
   ...
   task body Buffer is
	...
	:= (0, 0, 0, 0, 0);
      Assembly_Content: array(Assembly_Type, Product_Type) of Integer --przepisy
	:= ((2, 1, 2, 1, 2),
	    (2, 2, 0, 1, 0),
	    (1, 1, 2, 0, 1));
   ...
   begin
      ...
      Setup_Variables;
      loop
    select --dowolnie brac take i deliver bez kolejnosci - ktory zostal wywolamny
	 ...
	 or
        ...
        exception
                  when Ex =>
                     Put_Line("EXEPTION: assembly can't be delivered");
               end;
	 end Deliver;
	end select;
      end loop;
